...
fn block_skip<'r, 't>(parser: &mut Parser<'r, 't>) -> ParseResult<'r, 't, Elements<'t>> {
    ...
    if result {
        ...
        Err(parser.make_warn(ParseErrorKind::RuleFailed))
    }
}
    ...
    'r: 't,
{
    ...
    let block = match get_block_rule_with_name(name) {
        Some(block) => block,
        None => return Err(parser.make_warn(ParseErrorKind::NoSuchBlock)),
    };
    ...
    if !block.accepts_star && flag_star {
        return Err(parser.make_warn(ParseErrorKind::BlockDisallowsStar));
    }

    // Check if this block allows score invocation ('_' after name)
    if !block.accepts_score && flag_score {
        return Err(parser.make_warn(ParseErrorKind::BlockDisallowsScore));
    }
