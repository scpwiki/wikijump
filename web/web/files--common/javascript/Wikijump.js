...
Wikijump.utils.formatDates = function(topElementId){
 ...
 */
Wikijump.utils.handleError = function(r){
	if(r.status != 'ok'){
...
Wikijump.render = {};
Wikijump.render.printuser = function(userId, userName, wImage){
	var link = 'href="javascript:;" onclick="Wikijump.page.listeners.userInfo('+userId+')"';
...
Wikijump.render.fixAvatarHover = function(root){
	...
	for(var i = 0; i<users.length; i++){
		...
		if(a.getElementsByTagName("img").length == 1){
			...
			YAHOO.util.Event.addListener(a, "mouseover", Wikijump.render.fixAvatarHover.showHover);
		}
...
Wikijump.render.fixAvatarHover.showHover = function(e){
	...
	if(this.hoverAvatar == null){
		...
		this.hoverAvatar = a;
		ac.appendChild(a);
		YAHOO.util.Event.addListener(a, "mouseout", Wikijump.render.fixAvatarHover.hideHover);
		YAHOO.util.Event.addListener(a, "mousemove", Wikijump.render.fixAvatarHover.mousemove);
	...
	setTimeout('Wikijump.render.fixAvatarHover.mousemove.autoHide("'+ha.id+'")', 1000);

}

Wikijump.render.fixAvatarHover.hideHover= function(e){
...
Wikijump.render.fixAvatarHover.mousemove = function(e){
	this.lastAccess = (new Date()).getTime();

}

Wikijump.render.fixAvatarHover.mousemove.autoHide= function(hoverId){
	var a = $(hoverId);
	...
	if(a.lastAccess +3000 < now){
		...
		// check again
		setTimeout('Wikijump.render.fixAvatarHover.mousemove.autoHide("'+a.id+'")', 1000);
