...
const OZONE = {
    ...
    requestModuleCallback: {
      success: function (rObj) {
        ...
        if (response.jsInclude != null) {
          for (let index = 0; index < response.jsInclude.length; index++) {
            OZONE.utils.addJavascriptUrl(response.jsInclude[index])
          }
        }
        if (response.cssInclude != null) {
          for (let index = 0; index < response.cssInclude.length; index++) {
  ...
  utils: {
    ...
    formatDates: function (topElement) {
      ...
      for (let i = 0; i < dates.length; i++) {
        ...
        if (inner.match(/^[0-9]+\s*\|.*$/)) {
          var nu
          // with formatting
          const timestamp = inner.replace(/^([0-9]+)\s*\|.*/, '$1')
          ...
          if (dstring.match(/%O/) || options.match(/agohover/)) {
            ...
            if (options.match(/agohover/)) {
              ...
              YAHOO.util.Event.addListener(dates[i], 'mouseover', function (_event) {
    ...
    formatDatesOld: function (topElementId) {
      ...
      if (topElementId === null) {
        dates = document.getElementsByTagName('odate')
      } else {
        var el = $(topElementId)
        dates = el.getElementsByTagName('odate')
      }
      for (let i = 0; i < dates.length; i++) {
        // TODO: make it better ;-)
  ...
  visuals: {
    ...
    scrollToCenter: function (element, options) {
      ...
      if (options != null && options.blink === true) {
        const ofx34 = new fx.Opacity(element, { duration: 150, transition: fx.circ })
  ...
  forms: {
    lengthLimiter: function (textElement, countElement, limit) {
    ...
    // XXX This needs to be added back in somehow
  ...
  dom: {
    ...
    onDomReady: function (f, el, doc) {
      ...
      if (
        ...
        if (typeof f === 'function') { f() } else { OZONE.dom.onDomReady.fs[f].call() }
      } else {
        var fid
        if (typeof f === 'function') {
          if (!OZONE.dom.onDomReady.fs) {
            OZONE.dom.onDomReady.fs = []
          }
