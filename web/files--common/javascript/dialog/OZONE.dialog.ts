...
type HovertipElement = HTMLElement & {
...
export const dialog = {
  ...
  tempEffect: null as null | unknown,

  cleanAll: function (options?: { timeout: number }): void {
    /**
    ...
    setTimeout(() => dialog.factory.boxcontainer().hide(), timeout);
    setTimeout(() => dialog.factory.shader().hide(), timeout);
  },

  factory: {
    shader: function (): unknown {
      if (dialog.factory.stock.shader === null) {
        dialog.factory.stock.shader = new dialog.Shader();
      }
      return dialog.factory.stock.shader;
    },
    boxcontainer: function (): unknown {
      if (dialog.factory.stock.boxcontainer === null) {
        dialog.factory.stock.boxcontainer = new dialog.BoxContainer();
      }
      return dialog.factory.stock.boxcontainer;
    },
    stock: {
      shader: null as null | unknown,
      boxcontainer: null as null | unknown
    }
  },
  ...
  BoxContainer: class {
    ...
    attachDD (): void {
       ...
       * ?? (unused)
       */
      const tDivs = this.dialogElement.getElementsByTagName('div');
      for (const i in tDivs) {
        ...
        if (tDivs[i].className === "close") {
          // tDivs[i].id="oclose-1";
          YAHOO.util.Event.addListener(tDivs[i], "click", dialog.cleanAll);
        }
      }
    }
  ...
  hovertip: {
    ...
    makeTip: function (
      ...
      if (typeof element !== "string") {
        if (isHtmlCollection(element)) {
          ...
          Array.from(element).forEach((element: HTMLElement) => {
            dialog.hovertip.makeTip(element, options);
          });
          return;
        } else {
          el = element;
      ...
      if (options && options.context) {
        ...
        tipEl = document.getElementById(options.context);
        if (!tipEl) {
    ...
    _mouseover: function (e: Event): void {
      /**
    ...
    dominit: function (topEl: string, options: HovertipOptions): void {
      /**
       * ??
       * something to do with readying the dom for hovertips
       *
       * @param topEl: The ID of the element to be used as the starting point
       * for searching. ??
       * TODO Some calls pass an element to this
       * @param options: ??
       */
      ...
      dialog.hovertip.init(); // just for sure
      let allDivs: HTMLCollection;
      if (topEl) {
        allDivs = document.getElementById(topEl).getElementsByTagName('div');
      } else {
    ...
    hideAll: function (): void {
      ...
      Array.from(
        document.getElementById('odialog-hovertips')
          .getElementsByClassName('hovertip')
      ).forEach((hovertip: HTMLElement) => {
