...
export function gestureObserve(target: HTMLElement, handler: (gesture: Gesture) => void) {
  ...
  const cancel = () => {
    id = null
    start = null
    document.removeEventListener("touchmove", wrapper)
    document.removeEventListener("touchend", wrapper)
    document.removeEventListener("touchcancel", wrapper)
  }
  ...
  const wrapper = (evt: TouchEvent) => {
    ...
    if (evt.type === "touchstart") {
      document.addEventListener("touchmove", wrapper, { passive: true })
      document.addEventListener("touchend", wrapper, { passive: true })
      document.addEventListener("touchcancel", wrapper, { passive: true })
      touch = evt.changedTouches[0]
      id = touch.identifier
    ...
    if (id !== null && start) {
      ...
      const gesture = new Gesture(start, [touch.clientX, touch.clientY], type)

      if (type !== "start") {
        const target = evt.target as HTMLElement
        // if we found a scrollable element in the direction of our gesture, cancel
        if (scrollElement(target, gesture.axis)) cancel()
      }
