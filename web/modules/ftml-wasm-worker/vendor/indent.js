// FROM: https://github.com/zebzhao/indent.js
// This file is a modified version of the source for indent.js
// The original doesn't play nice in a web worker, which this one does.
// This version also has been cleaned up, e.g. ESM configuration.

var rulesCache = {}

function filterRules(language, rules, excludes) {
  if (rulesCache[language]) return rulesCache[language]
  var ret = []
  rulesCache[language] = ret
  excludes = excludes || ""
  for (var i = 0; i < rules.length; i++) {
    if (
      rules[i].$languages.indexOf(language.toLowerCase()) !== -1 &&
...
/**
 * Soft dedent: this type of dedent has the opposite effect and will
 * actually indent every line starting from the opening line.
 */

/**
 * $indent - whether rule will cause indent $ignoreRules - ignore further
 * rule matching as long as this is last active rule, e.g. string, comments
 * $consumeEndMatch - advance the cursor to the end of the end pattern
 * matches $endPatternIndent - keep the indent rule active for the
 * $endPatterns $endPatterns - list of regex to terminate the rule
 * $startPatterns - list of regex to start the rule $matchBeginning - match
 * at beginning of line only $languages - used to filter by language later
 * $lineOffset - added to the line field when rule is applied $excludeIf -
 * used to exclude rule matching if any of these rules are active $lastRule
 * - used to continue a previous rule $newScope - used to determine if rule
 * creates a new scope, used for lastRule
 *
 * Always keep NEW_LINE_REGEX $endPatterns as last element, as otherwise it
 * will be matched first, and subsequent ones may be ignored and skipped
 * permanently by other rules.
 */
var MASTER_RULES = [
  {
    $languages: "js html",
...
function indent(code, baseRules, options) {
   ...
   *
   * IndentDeltas - store the the deltas in tabString - can be manipulated
   * directly to alter the tabString indentBuffer - used to keep tabs on
   * the number of open indentations on each line dedentBuffer - each line
   * in the buffer has an array storing open indent lines to be closed - an
   * array of numbers is used to reference the opening line - a negative
   * number is used to signify a soft dedent (see note about soft dedent)
   *
   * Each line can create at most 1 tabString. When a line is 'used up' for
   * dedent, it cannot be used again, hence the indentBuffer.
   */
  var tabString = options && options.tabString != null ? options.tabString : "\t"
  var lines = code.split(/[\r]?\n/gi)
  var lineCount = lines.length
  var ignoreBuffer = new Int16Array(lineCount)
  var indentBuffer = new Int16Array(lineCount)
  ...
  var hardIndentCount,
    ...
    j,
    indents = 0,
    hardIndents = new Int16Array(indentBuffer),
    indentDeltas = new Int16Array(lineCount),
    newLines = []
  ...
  for (i = 0; i < lineCount; i++) {
    if (ignoreBuffer[i - 1] === 1 && ignoreBuffer[i] === 1) {
      ...
      newLines.push((indents > 0 ? tabString.repeat(indents) : "") + lines[i].trim())
    }
  }
  ...
  function removeCurrentRule() {
    ...
    if (rule.$indent) {
      var endPatternIndent =
        typeof rule.$endPatternIndent === "function"
          ? rule.$endPatternIndent(matchEnd)
          : rule.$endPatternIndent
  ...
  function matchStartRule(string, rules, index) {
    ...
    for (var rule, r = 0; r < rules.length; r++) {
      ...
      if (
        rule.$excludeIf &&
        rule.$excludeIf.some(function (excludeRule) {
...
function cleanEscapedChars(string) {
  return string.replace(
    /\\(u[0-9A-Za-z]{4}|u\{[0-9A-Za-z]{1,6}]\}|x[0-9A-Za-z]{2}|.)/g,
    "0"
  )
}
...
function nonWhitespaceFollowByNewline(string, rule, state, matchStart) {
  ...
  if (!state.newline) {
    ...
    state.newline =
      index !== -1 &&
      (index <= matchStart.relativeIndex || matchStart.relativeIndex === -1)
  } else {
...
export function js(code, options) {
  return indent(code, filterRules("js", MASTER_RULES), options)
}

export function ts(code, options) {
  return indent(code, filterRules("js", MASTER_RULES), options)
}

export function html(code, options) {
  var rules =
    options && options.indentHtmlTag
      ? filterRules("html", MASTER_RULES, "html-tag")
      : filterRules("html", MASTER_RULES)
