...
test("cleanup refuses content or identity changed after creation", async () => {
  ...
  await assert.rejects(adapter.remove(resource, {expected: {title: resource.title, source_sha256: resource.source_sha256}, identity}), /identity, title, or source changed/);
...
backend.inspect = lambda slug: None
...
class Backend:
    ...
    def inspect(self, slug):
        self.count += 1
        return {"identity": self.count, "title": "fixture", "source_sha256": "0" * 64}
    def close(self): pass
