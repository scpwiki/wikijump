    ...
    'r: 't,
{
    ...
    pub fn get_head_map(
        ...
        if in_head {
            ...
            loop {
                ...
                self.get_optional_space()?;
                let (value, head_finished) = self.get_head_argument_value()?;
    ...
    fn get_head_argument_value(&mut self) -> Result<(Cow<'t, str>, bool), ParseError> {
        if self.current().token == Token::DoubleQuote {
            return self
                .get_quoted_string(RULE_BLOCK)
                .map(|value| (value, false));
        }

        if self.current().slice != "#" {
            return Err(self.make_err(ParseErrorKind::BlockMalformedArguments));
        }

        // Wikidot components commonly emit anchor links as `link=#`. Keep the
        // existing quoted-only behavior for other block argument values.
        let (value, last) = collect_text_keep(
            self,
            RULE_BLOCK,
            &[
                ParseCondition::current(Token::Whitespace),
        ...
        Ok((Cow::Borrowed(value), last.token == Token::RightBlock))
    }
