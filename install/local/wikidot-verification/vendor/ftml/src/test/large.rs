...
/// Inactive `iftags` bodies should be skipped without parsing.
#[test]
fn false_iftags_skips_deep_body() {
    let page_info = PageInfo::dummy();
    let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);

    let mut input = String::from("[[iftags +component]]\n");
    for _ in 0..101 {
        input.push_str("[[div]]\n");
    }
    for _ in 0..101 {
...
/// Inactive `ifcategory` bodies should be skipped without parsing.
#[test]
fn false_ifcategory_skips_deep_body() {
    let page_info = PageInfo::dummy();
    let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);

    let mut input = String::from("[[ifcategory component]]\n");
    for _ in 0..101 {
        input.push_str("[[div]]\n");
    }
    for _ in 0..101 {
...
/// Stray closing bibliography-cite parentheses should render as text.
///
/// This is common in informal text such as emoticons, and must not enter the
/// bibcite parser because that parser expects the opening token.
#[test]
fn stray_right_parentheses_inside_italics() {
    let page_info = PageInfo::dummy();
    let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);
