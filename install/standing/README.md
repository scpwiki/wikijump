...
2. Build the Deepwell, Framerail, and WWS images from the exact merged head under the appropriate exclusive build lease. Build the Framerail standing image with `--build-arg FRAMERAIL_ENV=local` so its SvelteKit CSP is compiled for the local runtime. Record their digests and FTML pin, then render the canonical host home with `STANDING_PROJECT_NAME=wikijump-standing` and `STANDING_NETWORK_NAME=wikijump-standing_default`.
