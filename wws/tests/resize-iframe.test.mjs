...
function runResizeScriptWithParent(hash, parent) {
  vm.runInNewContext(
    embeddedResizeScript(),
    {location: {hash, toString() { return this.hash; }}, parent},
    {filename: "resize-iframe.html"},
  );
}
...
test("malformed fragments and absent src attributes fail closed", () => {
  for (const hash of ["", "#", "#42", "#42/", "#-1/1", "#42/one/two", "#abc/1"]) {
    const result = runResizeScript(hash, ["/page/html/1"]);
    assert.deepEqual(result.selectors, [], hash);
    assert.equal(result.elements[0].height, null, hash);
  }

  const absentSrc = runResizeScript("#42/1", [null, "", "/page/html/1"]);
  assert.deepEqual(absentSrc.elements.map(({height}) => height), [null, null, "42px"]);
});

test("cross-origin parent access cannot reach an iframe collection", () => {
  const parent = {};
  ...
  assert.throws(
    () => runResizeScriptWithParent("#42/1", parent),
    (error) => error.name === "SecurityError",
  );
});

test("the bridge requires the parent page's jQuery alias", () => {
  assert.throws(
    () => runResizeScriptWithParent("#42/1", {parent: {}}),
    /\$j is not a function/,
  );
