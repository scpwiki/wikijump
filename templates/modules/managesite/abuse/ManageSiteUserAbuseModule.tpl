	...
	<table  class="form alignleft">
		...
		{foreach from=$reps item=rep}
			<tr>
				...
				<td>
					| <a href="javascript:;" onclick="WIKIDOT.modules.ManageSiteUserAbuseModule.listeners.clear(event, '{$rep.user->getUserId()}')">clear flags</a>
					{if $rep.member}
						| <a href="javascript:;" onclick="WIKIDOT.modules.ManageSiteUserAbuseModule.listeners.removeUser({$rep.user->getUserId()}, '{$rep.user->getNickName()|escape}')">remove from members</a>
						| <a href="javascript:;" onclick="WIKIDOT.modules.ManageSiteUserAbuseModule.listeners.removeAndBan({$rep.user->getUserId()}, '{$rep.user->getNickName()|escape}')">remove from members &amp; ban</a>
					{else}
						| <a href="javascript:;"  onclick="WIKIDOT.modules.ManageSiteUserAbuseModule.listeners.banUser({$rep.user->getUserId()}, '{$rep.user->getNickName()|escape}')">ban</a>
					{/if}
				</td>
			</tr>
		{/foreach}
