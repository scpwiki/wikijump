...
use self::literal_regions::WikidotLiteralRegionScanner;
...
impl RenderService {
    ...
    fn protect_wikidot_wikipedia_links(
        ...
        let mut literal_regions = WikidotLiteralRegionScanner::default();
        ...
        for captures in WIKIDOT_WIKIPEDIA_LINK_REGEX.captures_iter(&source) {
            ...
            if literal_regions.is_inside(&source, link_match.start()) {
                output.push_str(link_match.as_str());
                continue;
            }
...
mod tests {
    ...
    fn restores_dense_wikidot_wikipedia_links_without_repeated_scans() {
        ...
        let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);
        let mut wikitext = String::with_capacity(LINK_COUNT * 48);
