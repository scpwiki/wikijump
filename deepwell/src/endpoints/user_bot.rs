...
pub async fn bot_user_create(
    ...
    for owner_user_id in owners {
        let owner_user = UserService::get_wikijump(ctx, Reference::Id(owner_user_id))
            .await
            .or_raise(make_error)?;
...
pub async fn bot_user_get_owners(
    ...
    let user_bot = UserService::get_wikijump_optional(ctx, reference)
        .await
        .or_raise(make_error)?;
...
pub async fn bot_user_get_bots(
    ...
    let owner_user = UserService::get_wikijump(ctx, reference)
        .await
        .or_raise(make_error)?;
...
pub async fn bot_user_owner_set(
    ...
    let bot_user = UserService::get_wikijump(ctx, Reference::Id(input.bot_user_id))
        .await
        .or_raise(make_error)?;
