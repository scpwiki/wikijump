 ...
 * @param anyIndex - Whether to include elements with `tabindex` set to
 *   `-1`.
 */
...
export class FocusGroup {
  ...
  constructor(
    ...
    public direction: FocusGroupDirection
  ) {
...
export interface FocusOpts {
  ...
  focus?: () => void
  /**
   * Callback fired when the element and its children are no longer
   * focused.
   */
  blur?: () => void
}
